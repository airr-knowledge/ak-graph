...
build-graph:
	python graph_analysis_using_networkit.py build_graph $(LOCUS) --VERSION $(VERSION) --WORKDIR $(WORKDIR)

#set analysis type to graph_stat
graph-stat:
	python graph_analysis_using_networkit.py graph_stat $(LOCUS) --VERSION $(VERSION) --WORKDIR $(WORKDIR)

get-epitope-info:
	python get_epitope_information.py $(LOCUS) --VERSION $(VERSION)

get-epitope-info-tra:
	python get_epitope_information.py tra --VERSION v1
get-epitope-info-trb:
	python get_epitope_information.py trb --VERSION v1
get-epitope-info-trd:
	python get_epitope_information.py trd --VERSION v1
get-epitope-info-trg:
	python get_epitope_information.py trg --VERSION v1
get-epitope-info-igh:
	python get_epitope_information.py igh --VERSION v1
get-epitope-info-igk:
	python get_epitope_information.py igk --VERSION v1
get-epitope-info-igl:
	python get_epitope_information.py igl --VERSION v1

get-all-epitope-info: get-epitope-info-tra get-epitope-info-trb get-epitope-info-trd get-epitope-info-trg

epitope-analysis:
	python CDR3AndEpitopeAnlysis.py
...
cdr3-dist1-edgelist: stream_query
	$(BIN_DIR)/stream_query $(LOCUS) $(VERSION) $(WORKDIR)
...
cdr3-edgelist-threads: stream_query_threads
	$(BIN_DIR)/stream_query_threads $(LOCUS) $(VERSION) $(WORKDIR) $(N_THREADS)
